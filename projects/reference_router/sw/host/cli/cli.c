...
void addmac(int port, uint8_t *mac) {
  ...
  err=regwrite(nf10,MAC_HI_REGS[port], mac[0] << 8 | mac[1]);
  if(err) printf("0x%08x: ERROR\n", MAC_HI_REGS[port]);
  err=regwrite(nf10,MAC_LO_REGS[port], mac[2] << 24 | mac[3] << 16 | mac[4] << 8 | mac[5]);
  if(err) printf("0x%08x: ERROR\n", MAC_LO_REGS[port]);
}
...
void addarp(int entry, uint8_t *ip, uint8_t *mac) {
  ...
  err=regwrite(nf10,ROUTER_OP_LUT_ARP_TABLE_ENTRY_NEXT_HOP_IP_REG, ip[0] << 24 | ip[1] << 16 | ip[2] << 8 | ip[3]);
  if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ARP_TABLE_ENTRY_NEXT_HOP_IP_REG);
  err=regwrite(nf10,ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_HI_REG,  mac[0] << 8 | mac[1]);
  if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_HI_REG);
  err=regwrite(nf10,ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_LO_REG, mac[2] << 24 | mac[3] << 16 | mac[4] << 8 | mac[5]);
  if(err) printf(nf10,"0x%08x: ERROR\n", ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_LO_REG);
  err=regwrite(nf10,ROUTER_OP_LUT_ARP_TABLE_WR_ADDR_REG, entry);
...
void addip(int entry, uint8_t *subnet, uint8_t *mask, uint8_t *nexthop, int port) {
  ...
  err=regwrite(nf10,ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_IP_REG, subnet[0] << 24 | subnet[1] << 16 | subnet[2] << 8 | subnet[3]);
  if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_IP_REG);
  err=regwrite(nf10,ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_MASK_REG, mask[0] << 24 | mask[1] << 16 | mask[2] << 8 | mask[3]);
  if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_MASK_REG);
  err=regwrite(nf10,ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_NEXT_HOP_IP_REG, nexthop[0] << 24 | nexthop[1] << 16 | nexthop[2] << 8 | nexthop[3]);
  if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_NEXT_HOP_IP_REG);
  err=regwrite(nf10,ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_OUTPUT_PORT_REG, port);
  if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_OUTPUT_PORT_REG);
  err=regwrite(nf10,ROUTER_OP_LUT_ROUTE_TABLE_WR_ADDR_REG, entry);
...
void listip(void) {
  ...
  for (i = 0; i < ROUTER_OP_LUT_ROUTE_TABLE_DEPTH; i++) {
    ...
    err=regwrite(nf10,ROUTER_OP_LUT_ROUTE_TABLE_RD_ADDR_REG, i);
    if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ROUTE_TABLE_RD_ADDR_REG);
    err=regread(nf10,ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_IP_REG, &subnet);
    if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_IP_REG);
    err=regread(nf10,ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_MASK_REG, &mask);
    if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_MASK_REG);
    err=regread(nf10,ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_NEXT_HOP_IP_REG, &nh);
    if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_NEXT_HOP_IP_REG);
    err=regread(nf10,ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_OUTPUT_PORT_REG, &valport);
    if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_OUTPUT_PORT_REG);
...
void listarp(void) {
  ...
  for (i = 0; i < ROUTER_OP_LUT_ARP_TABLE_DEPTH; i++) {
    ...
    err=regwrite(nf10,ROUTER_OP_LUT_ARP_TABLE_RD_ADDR_REG, i);
    if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ARP_TABLE_RD_ADDR_REG);
    err=regread(nf10,ROUTER_OP_LUT_ARP_TABLE_ENTRY_NEXT_HOP_IP_REG, &ip);
    if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ARP_TABLE_ENTRY_NEXT_HOP_IP_REG);
    err=regread(nf10,ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_HI_REG, &machi);
    if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_HI_REG);
    err=regread(nf10,ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_LO_REG, &maclo);
    if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_LO_REG);
...
void listmac(void) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    err=regread(nf10,MAC_HI_REGS[i], &machi);
    if(err) printf("0x%08x: ERROR\n", MAC_HI_REGS[i]);
    err=regread(nf10,MAC_LO_REGS[i], &maclo);
    if(err) printf("0x%08x: ERROR\n", MAC_LO_REGS[i]);
...
void clearip(void) {
  ...
  err=regwrite(nf10,ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_IP_REG, 0);
  if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_IP_REG);
  err=regwrite(nf10,ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_MASK_REG, 0xffffffff);
  if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_MASK_REG);
  err=regwrite(nf10,ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_NEXT_HOP_IP_REG, 0);
  if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_NEXT_HOP_IP_REG);
  err=regwrite(nf10,ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_OUTPUT_PORT_REG, 0);
  if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_OUTPUT_PORT_REG);
  err=regwrite(nf10,ROUTER_OP_LUT_ROUTE_TABLE_WR_ADDR_REG, entry);
  if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ROUTE_TABLE_WR_ADDR_REG);
...
void cleararp(void) {
  ...
  err=regwrite(nf10,ROUTER_OP_LUT_ARP_TABLE_ENTRY_NEXT_HOP_IP_REG, 0);
  if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ARP_TABLE_ENTRY_NEXT_HOP_IP_REG);
  err=regwrite(nf10,ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_HI_REG, 0);
  if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_HI_REG);
  err=regwrite(nf10,ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_LO_REG, 0);
  if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_LO_REG);
  err=regwrite(nf10,ROUTER_OP_LUT_ARP_TABLE_WR_ADDR_REG, entry);
  if(err) printf("0x%08x: ERROR\n", ROUTER_OP_LUT_ARP_TABLE_WR_ADDR_REG);
