...
// following function modified by sfriederich

int return_val;

  int
 main_swctl(int argc, char **argv)
 {
   int fd;
   off_t off, dp;
   ...
   for (c = commands; c->name; c++)
	 ...
	 // free the mapped memory
         if (munmap(p, sizeof(OCCP_Space) ) == -1) {
           printf("Error unmapping the file p \n");
         }
         if (munmap(d, sizeof(OCDP_Space) * 2 ) == -1) {
           printf("Error unmapping the file p \n");
         }
         close(fd);
         if (*ap = "wread") {
            // retur value read from flash
            return (return_val);
         }
         else {
           return 0;
