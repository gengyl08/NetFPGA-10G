...
void write_arp_cache_to_hw(router_state* rs) {
	...
	while((cur != NULL) && (i < ROUTER_OP_LUT_ARP_TABLE_DEPTH)) {
		...
		if(entry->is_static) {
			write_arp_cache_entry_to_hw(entry, i);
			i++;
		}
	...
	while(i < ROUTER_OP_LUT_ARP_TABLE_DEPTH) {
		...
		if(cur) {
			...
			write_arp_cache_entry_to_hw(NULL, i);
			i++;
		}
	}
}
...
void write_arp_cache_entry_to_hw(arp_cache_entry *entry, int row) {
	int err;
	...
	if(entry != NULL) {
		...
		mac_hi |= ((unsigned int)entry->arp_ha[1]);
                err=nf10_reg_wr(ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_HI_REG, mac_hi);
		...
		mac_lo |= ((unsigned int)entry->arp_ha[5]);
                err=nf10_reg_wr(ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_LO_REG, mac_lo);
                if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_LO_REG, nl_geterror(err));

		/* write the next hop ip data */
                err=nf10_reg_wr(ROUTER_OP_LUT_ARP_TABLE_ENTRY_NEXT_HOP_IP_REG, ntohl(entry->ip.s_addr));
	...
	/* set the row */
        err=nf10_reg_wr(ROUTER_OP_LUT_ARP_TABLE_WR_ADDR_REG, row);
...
void cli_show_hw_arp_cache(router_state *rs, cli_request *req) {
	...
	if(rs->is_netfpga) {
		...
		sprint_hw_arp_cache(&arp_info, &len);
