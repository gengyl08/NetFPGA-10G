...
void sprint_hw_rtable(char **buffer, unsigned int *len) {
	...
	for(i=0; i < ROUTER_OP_LUT_ROUTE_TABLE_DEPTH; i++) {
		...
		/* read the four-tuple (ip, gw, mask, iface) from the hw registers */
                err=nf10_reg_rd(ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_IP_REG, &ip.s_addr);
                if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_IP_REG, nl_geterror(err));
                err=nf10_reg_rd(ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_MASK_REG, &mask.s_addr);
                if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_MASK_REG, nl_geterror(err));
                err=nf10_reg_rd(ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_NEXT_HOP_IP_REG, &gw.s_addr);
                if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_NEXT_HOP_IP_REG, nl_geterror(err));
                err=nf10_reg_rd(ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_OUTPUT_PORT_REG, &port);
...
void sprint_hw_arp_cache(char **buf, unsigned int *len) {
	...
	for(i=0; i < ROUTER_OP_LUT_ARP_TABLE_DEPTH; i++) {
		...
		/* read the four-touple (mac hi, mac lo, gw, num of misses) */
                err=nf10_reg_rd(ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_HI_REG, &mac_hi);
                if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_HI_REG, nl_geterror(err));
                err=nf10_reg_rd(ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_LO_REG, &mac_lo);
                if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_ARP_TABLE_ENTRY_MAC_LO_REG, nl_geterror(err));
                err=nf10_reg_rd(ROUTER_OP_LUT_ARP_TABLE_ENTRY_NEXT_HOP_IP_REG, &gw.s_addr);
...
void sprint_hw_iface(router_state *rs, char **buf, unsigned int *len) {
	...
	for(i=0; i<4; i++) {
		...
		read_hw_iface_mac(i, &mac_hi, &mac_lo);
...
void sprint_hw_nat_table(char **buf, unsigned int *len) {
...
//	int err;
        ...
        for(i=0; i<16; i++) {
                ...
                /* write the row number */
                //err=nf10_reg_wr(ROUTER_OP_LUT_NAT_RD_ADDR_REG, i);
                //if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_RD_ADDR_REG, nl_geterror(err));

                /* read the 7-tuple (e-ip, e-port, e-sum, i-ip, i-port, i-sum, hits) from hw registers */
                //err=nf10_reg_rd(ROUTER_OP_LUT_NAT_INT_IP_REG, &int_ip.s_addr);
                //if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_INT_IP_REG, nl_geterror(err));
                //err=nf10_reg_rd(ROUTER_OP_LUT_NAT_INT_PORT_REG, &int_port);
                //if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_INT_PORT_REG, nl_geterror(err));
                //err=nf10_reg_rd(ROUTER_OP_LUT_NAT_INT_CHKSUM_REG, &int_sum);
                //if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_INT_CHKSUM_REG, nl_geterror(err));
                //err=nf10_reg_rd(ROUTER_OP_LUT_NAT_EXT_IP_REG, &ext_ip.s_addr);
                //if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_EXT_IP_REG, nl_geterror(err));
                //err=nf10_reg_rd(ROUTER_OP_LUT_NAT_EXT_PORT_REG, &ext_port);
                //if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_EXT_PORT_REG, nl_geterror(err));
                //err=nf10_reg_rd(ROUTER_OP_LUT_NAT_EXT_CHKSUM_REG, &ext_sum);
                //if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_EXT_CHKSUM_REG, nl_geterror(err));
                //err=nf10_reg_rd(ROUTER_OP_LUT_NAT_HIT_REG, &hits);
                //if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_HIT_REG, nl_geterror(err));
...
void sprint_hw_drops(router_state *rs, char **buf, unsigned int *len) {
	...
	if (rs->is_netfpga) {
		...
		snprintf(line, HW_DROPS_LEN, "%4s %10u %4s %10u %4s %10u %4s %10u\n",
			port_names[0], get_rx_queue_num_pkts_dropped_full(0) + get_rx_queue_num_pkts_dropped_bad(0),
			port_names[1], get_rx_queue_num_pkts_dropped_full(1) + get_rx_queue_num_pkts_dropped_bad(1),
			port_names[2], get_rx_queue_num_pkts_dropped_full(2) + get_rx_queue_num_pkts_dropped_bad(2),
			port_names[3], get_rx_queue_num_pkts_dropped_full(3) + get_rx_queue_num_pkts_dropped_bad(3));
...
void sprint_hw_oq_drops(router_state *rs, char **buf, unsigned int *len) {
	...
	if (rs->is_netfpga) {
		...
		snprintf(line, HW_OQ_DROPS_LEN, "%4s %10u %4s %10u %4s %10u %4s %10u\n",
			port_names[0], get_oq_num_pkts_dropped(0),
			port_names[1], get_oq_num_pkts_dropped(1),
			port_names[2], get_oq_num_pkts_dropped(2),
			port_names[3], get_oq_num_pkts_dropped(3));
		COPY_STRING(buffer, total_len, line);

		snprintf(line, HW_OQ_DROPS_LEN, "%4s %10u %4s %10u %4s %10u %4s %10u\n",
			port_names[4], get_oq_num_pkts_dropped(4),
			port_names[5], get_oq_num_pkts_dropped(5),
			port_names[6], get_oq_num_pkts_dropped(6),
			port_names[7], get_oq_num_pkts_dropped(7));
		COPY_STRING(buffer, total_len, line);
	}
...
void sprint_hw_local_ip_filter(router_state *rs, char **buf, unsigned int *len) {
	...
	if (rs->is_netfpga) {
		...
		for (i = 0; i < ROUTER_OP_LUT_DST_IP_FILTER_TABLE_DEPTH; ++i) {
                	...
                	err=nf10_reg_rd(ROUTER_OP_LUT_DST_IP_FILTER_TABLE_ENTRY_IP_REG, &ip.s_addr);
