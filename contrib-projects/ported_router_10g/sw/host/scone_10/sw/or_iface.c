...
void read_hw_iface_mac(unsigned int port, unsigned int *mac_hi, unsigned int *mac_lo) {

	int err;
	switch(port) {

		case 0:
                	err=nf10_reg_rd(ROUTER_OP_LUT_MAC_0_HI_REG, mac_hi);
                	if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_MAC_0_HI_REG, nl_geterror(err));
                	err=nf10_reg_rd(ROUTER_OP_LUT_MAC_0_LO_REG, mac_lo);
                	if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_MAC_0_LO_REG, nl_geterror(err));
			break;

		case 1:
                        err=nf10_reg_rd(ROUTER_OP_LUT_MAC_1_HI_REG, mac_hi);
                        if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_MAC_1_HI_REG, nl_geterror(err));
                        err=nf10_reg_rd(ROUTER_OP_LUT_MAC_1_LO_REG, mac_lo);
                        if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_MAC_1_LO_REG, nl_geterror(err));
			break;

		case 2:
                        err=nf10_reg_rd(ROUTER_OP_LUT_MAC_2_HI_REG, mac_hi);
                        if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_MAC_2_HI_REG, nl_geterror(err));
                        err=nf10_reg_rd(ROUTER_OP_LUT_MAC_2_LO_REG, mac_lo);
                        if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_MAC_2_LO_REG, nl_geterror(err));
			break;
		case 3:
                        err=nf10_reg_rd(ROUTER_OP_LUT_MAC_3_HI_REG, mac_hi);
                        if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_MAC_3_HI_REG, nl_geterror(err));
                        err=nf10_reg_rd(ROUTER_OP_LUT_MAC_3_LO_REG, mac_lo);
...
void write_hw_iface_mac(unsigned int port, unsigned int mac_hi, unsigned int mac_lo) {
	int err;
	switch(port) {
...
void cli_hw_interface_add(router_state* rs, cli_request* req) {
	...
	if(rs->is_netfpga) {
		...
		write_hw_iface_mac(port, mac_hi, mac_lo);
...
void set_hw_iface(unsigned int queue, unsigned int command) {

//	int err;
/*	switch(queue) {
		case 0:
                        err=nf10_reg_wr(MAC_GRP_0_CONTROL_REG, command);
                        if(err) printf("0x%08x: ERROR: %s\n", MAC_GRP_0_CONTROL_REG, nl_geterror(err));
			break;
		case 1:
                        err=nf10_reg_wr(MAC_GRP_1_CONTROL_REG, command);
                        if(err) printf("0x%08x: ERROR: %s\n", MAC_GRP_1_CONTROL_REG, nl_geterror(err));
			break;
		case 2:
                        err=nf10_reg_wr(MAC_GRP_2_CONTROL_REG, command);
                        if(err) printf("0x%08x: ERROR: %s\n", MAC_GRP_2_CONTROL_REG, nl_geterror(err));
			break;
		case 3:
                        err=nf10_reg_wr(MAC_GRP_3_CONTROL_REG, command);
                        if(err) printf("0x%08x: ERROR: %s\n", MAC_GRP_3_CONTROL_REG, nl_geterror(err));
			break;
		default:
