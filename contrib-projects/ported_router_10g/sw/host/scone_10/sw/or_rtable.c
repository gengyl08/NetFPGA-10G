...
void write_rtable_to_hw(router_state* rs) {
	...
	for (i = 0; i < ROUTER_OP_LUT_ROUTE_TABLE_DEPTH; ++i) {
		...
		if (cur) {
			...
			/* write the ip */
                        err=nf10_reg_wr(ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_IP_REG, ntohl(entry->ip.s_addr));
                        if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_IP_REG, nl_geterror(err));
			/* write the mask */
                        err=nf10_reg_wr(ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_MASK_REG, ntohl(entry->mask.s_addr));
                        if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_MASK_REG, nl_geterror(err));
			/* write the next hop */
                        err=nf10_reg_wr(ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_NEXT_HOP_IP_REG, ntohl(entry->gw.s_addr));
                        if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_NEXT_HOP_IP_REG, nl_geterror(err));
			/* write the port */
                        err=nf10_reg_wr(ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_OUTPUT_PORT_REG, getOneHotPortNumber(entry->iface));
			...
			/* write the ip */
                        ...
                        if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_IP_REG, nl_geterror(err));
			/* write the mask */
                        err=nf10_reg_wr(ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_MASK_REG, 0);
                        if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_ROUTE_TABLE_ENTRY_MASK_REG, nl_geterror(err));
			/* write the next hop */
...
void cli_show_hw_rtable(router_state *rs, cli_request *req) {
	...
	if(rs->is_netfpga) {
		...
		sprint_hw_rtable(&hw_rtable_info, &len);
