...
void cli_nat_set(router_state *rs, cli_request *req) {
	...
	if( (strncmp(iface, ETH0, strlen(ETH0)) == 0) || (strncmp(iface, ETH1, strlen(ETH1)) == 0) ||
		...
		if (rs->is_netfpga) {
			/*
				int oneHotPort = getOneHotPortNumber(iface);
                                err=nf10_reg_wr(ROUTER_OP_LUT_NAT_WAN_INTERFACE, oneHotPort);
                                if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_WAN_INTERFACE, nl_geterror(err));
			*/
		}
		...
		if (rs->is_netfpga) {
                        //err=nf10_reg_wr(ROUTER_OP_LUT_NAT_WAN_INTERFACE, 0);
                	//if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_WAN_INTERFACE, nl_geterror(err));
		}
		...
		if (rs->is_netfpga) {
  		...
  		for(i=0; i<16; ++i) {
	  		if(cur) {
	    		...
	    		nat->hw_row = i;
					write_nat_table_entry_to_hw(nat, i);
					cur = cur->next;
				} else {
					write_nat_table_zero_to_hw(i);
				}
...
uint32_t get_hw_hits(uint8_t row) {

	uint32_t hits = 0;
	//int err;

        //err=nf10_reg_wr(ROUTER_OP_LUT_NAT_RD_ADDR_REG, row);
...
void write_nat_table_entry_to_hw(nat_entry *nat, uint8_t row) {

//	int err;
	/* write int ip */
	//err=nf10_reg_wr(ROUTER_OP_LUT_NAT_INT_IP_REG, ntohl(nat->nat_int.ip.s_addr));
        //if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_INT_IP_REG, nl_geterror(err));
	/* write int port */
        //err=nf10_reg_wr(ROUTER_OP_LUT_NAT_INT_PORT_REG, ntohs(nat->nat_int.port));
        //if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_INT_PORT_REG, nl_geterror(err));
	/* write int checksum */
        //err=nf10_reg_wr(ROUTER_OP_LUT_NAT_INT_CHKSUM_REG, ntohs(nat->nat_int.checksum));
        //if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_INT_CHKSUM_REG, nl_geterror(err));
	/* write ext ip */
        //err=nf10_reg_wr(ROUTER_OP_LUT_NAT_EXT_IP_REG, ntohl(nat->nat_ext.ip.s_addr));
        //if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_EXT_IP_REG, nl_geterror(err));
	/* write ext port */
        //err=nf10_reg_wr(ROUTER_OP_LUT_NAT_EXT_PORT_REG, ntohs(nat->nat_ext.port));
        //if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_EXT_PORT_REG, nl_geterror(err));
	/* write ext checksum */
        //err=nf10_reg_wr(ROUTER_OP_LUT_NAT_EXT_CHKSUM_REG, ntohs(nat->nat_ext.checksum));
        //if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_EXT_CHKSUM_REG, nl_geterror(err));
	/* write the number of hits */
        //err=nf10_reg_wr(ROUTER_OP_LUT_NAT_HIT_REG, ntohl(nat->hits));
        //if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_HIT_REG, nl_geterror(err));
	/* write the row number */
        //err=nf10_reg_wr(ROUTER_OP_LUT_NAT_WR_ADDR_REG, row);
        //if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_WR_ADDR_REG, nl_geterror(err));
}


void write_nat_table_zero_to_hw(uint8_t row) {
//	int err;
/*
        err=nf10_reg_wr(ROUTER_OP_LUT_NAT_INT_IP_REG, 0);
        if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_INT_IP_REG, nl_geterror(err));
        err=nf10_reg_wr(ROUTER_OP_LUT_NAT_INT_PORT_REG, 0);
        if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_INT_PORT_REG, nl_geterror(err));
        err=nf10_reg_wr(ROUTER_OP_LUT_NAT_INT_CHKSUM_REG, 0);
        if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_INT_CHKSUM_REG, nl_geterror(err));
        err=nf10_reg_wr(ROUTER_OP_LUT_NAT_EXT_IP_REG, 0);
        if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_EXT_IP_REG, nl_geterror(err));
        err=nf10_reg_wr(ROUTER_OP_LUT_NAT_EXT_PORT_REG, 0);
        if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_EXT_PORT_REG, nl_geterror(err));
        err=nf10_reg_wr(ROUTER_OP_LUT_NAT_EXT_CHKSUM_REG, 0);
        if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_EXT_CHKSUM_REG, nl_geterror(err));
        err=nf10_reg_wr(ROUTER_OP_LUT_NAT_HIT_REG, 0);
        if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_HIT_REG, nl_geterror(err));
        err=nf10_reg_wr(ROUTER_OP_LUT_NAT_WR_ADDR_REG, row);
        if(err) printf("0x%08x: ERROR: %s\n", ROUTER_OP_LUT_NAT_WR_ADDR_REG, nl_geterror(err));
*/
