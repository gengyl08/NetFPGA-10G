...
void init_cli(struct sr_instance* sr) {
	...
	register_cli_command(&(rs->cli_commands), "hw pkts fwd", &cli_hw_num_pkts_fwd);
...
void process_packet(struct sr_instance* sr, const uint8_t * packet, unsigned int len, const char* interface) {
	...
	*/
	...
	switch(ntohs(ether_hdr->eth_type)) {
		...
		case ETH_TYPE_IP:
			printf(" ** -> Received IP packet of length %d on iface %s\n", len,interface);
			process_ip_packet(sr, packet, len, interface);
			break;

		case ETH_TYPE_ARP:
			printf(" ** -> Received ARP packet of length %d on iface %s\n", len,interface);
